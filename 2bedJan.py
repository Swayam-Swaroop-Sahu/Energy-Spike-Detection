...
def generate_energy_usage():
    ...
    for i in range(0, 24, 2):
        if 0 <= i < 8:
            usage.append(np.random.uniform(0.05, 0.2))
        elif 8 <= i < 12:
            usage.append(np.random.uniform(0.2, 0.5))
        elif 12 <= i < 18:
            usage.append(np.random.uniform(0.1, 0.4))
        elif 18 <= i < 24:
            usage.append(np.random.uniform(0.4, 1.0))
    return usage
...
for day in range(1, num_days + 1):
    ...
    daily_csv_filename = f"{month}_2bed_energy_consumption_day_{day}.csv"
    df.to_csv(daily_csv_filename, index=False)

# day avg
df_all = pd.concat([pd.read_csv(f"{month}_2bed_energy_consumption_day_{day}.csv") for day in range(1, num_days + 1)])
...
plt.title(f"Daily Average Energy Consumption vs {month} Monthly Average for 2 bed")
plt.legend()
plt.grid()
plt.show()
